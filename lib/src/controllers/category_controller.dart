...
class CategoryController extends ControllerMVC {
  ...
  void listenForFoodsByCategory({String id, String message}) async {
    ...
    stream.listen((Food _food) {
      setState(() {
        if(settingsRepo.deliveryAddress.value!=null&&_food.restaurant.latitude!=null){

          if(Distanc(context).calculateDistance(_food.restaurant.latitude, _food.restaurant.longitude)<=_food.restaurant.deliveryRange.toInt()){
            foods.add(_food);
          }}
        else{
          foods.add(_food);
